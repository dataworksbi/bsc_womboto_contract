...
/*
   Womoboto - A DataworksBI Company
      ∩~~~~~~∩ 
    ξ  ･    ･  ξ 
   ξ  　 〇　   ξ 
   ξ　   ~~　    ξ 
...
contract WombotoToken is ERC20("WombotoToken", "WOMBOTO"), Ownable {

    uint256 private constant CAP = 88000000e18;
    ...
    function canUnlockAmount(address _account) public view returns (uint256) {
        // When block number less than startReleaseBlock, no WOMBOTOs can be unlocked
        if (block.number < startReleaseBlock) {
            return 0;
        }
        // When block number more than endReleaseBlock, all locked WOMBOTOs can be unlocked
        else if (block.number >= endReleaseBlock) {
            return _locks[_account];
        }
        // When block number is more than startReleaseBlock but less than endReleaseBlock,
        // some WOMBOTOs can be released
        else
        {
    ...
    function unlock() external {
        require(_locks[msg.sender] > 0, "no locked WOMBOTOs");
    ...
    // @dev move WOMBOTOs with its locked funds to another account
    ...
    function delegateBySig(
        ...
        require(signatory != address(0), "WOMBOTO::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "WOMBOTO::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "WOMBOTO::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, "WOMBOTO::getPriorVotes: not yet determined");
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying WOMBOTOs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(block.number, "WOMBOTO::_writeCheckpoint: block number exceeds 32 bits");
